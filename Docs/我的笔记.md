...
<details>
<summary>[渲染层错误] Uncaught SyntaxError: Unexpected token '<'</summary>
今天登录“微信开发者工具”，git报出project.config.json被开发者工具修改了三行。然后小程序控制台就一直有错误。
之后在网上查阅得知 <b>把project.config.json的“useMultiFrameRuntime”改为true就</b>可以了。
不知道为啥就被微信开发者工具改了个这么玩意儿。
</details>
